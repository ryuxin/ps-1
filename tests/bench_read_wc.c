...
unsigned long p99_log_w[N_OPS] CACHE_ALIGNED;
...
void bench_read(void) {
	int i, id, ret = 0;
	unsigned long long s1, e1, cost, tot = 0, rcu_cost = 0;
	...
	qsort(p99_log, N_OPS, sizeof(unsigned long), cmpfunc);
	printf("thd %d read max %lu 99 %lu 99.9 %lu 99.99 %lu avg %llu\n", 
	       id, p99_log[0], p99_log[N_OPS/100], p99_log[N_OPS/1000], p99_log[N_OPS/10000], tot/N_OPS);
	return;
}
...
void bench_update(void) {
	...
	if (id == 1) {
		qsort(p99_log_w, iter, sizeof(unsigned long), cmpfunc);
		printf("thd %d update max %lu 99 %lu 99.9 %lu 99.99 %lu avg %llu\n", 
		       id, p99_log_w[0], p99_log_w[iter/100], p99_log_w[iter/1000], p99_log_w[iter/10000], tot/iter);
	}
